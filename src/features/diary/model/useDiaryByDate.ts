...
import { loadDiaryByDate, saveDiaryByDate } from "../api/storage";
...
export function useDiaryByDate(date: string) {
  ...
  useEffect(() => {
    setIsReady(false);
    (async () => {
      const loaded = await loadDiaryByDate(date);
      setState(loaded);
      setIsReady(true);
    })();
  }, [date]);
  ...
  const removeMeal = (mealId: MealId) => {
    setState((prev) => ({ meals: prev.meals.filter((m) => m.id !== mealId) }));
  };
